...
Since 2003, NYC's 311 system has been the main way New Yorkers get help from their city government for nonemergency issues. Whether it's a noise complaint, a broken streetlight, or a pothole that needs fixing, calling 311 is how most people interact with city services. The system has handled over 250 million requests since it started. But here's the big question: does 311 actually serve everyone equally?

Our team is looking at **equity in NYC 311 service delivery** to see if some groups get better treatment than others based on where they live, their income, or their demographics. My part of this project focuses on one specific question: **Which boroughs or neighborhoods show the biggest differences in 311 response times when you compare them by wealth?**

I think this matters for a few reasons. If rich neighborhoods get faster responses than poor ones, then 311 is actually making inequality worse instead of better. That's a social justice issue. Also, if we can figure out where the biggest gaps are, the city could use that information to send more help to the places that need it most. And looking at which types of complaints get handled faster or slower might tell us something about how the city decides what problems are urgent and what aren't.

### Analytical Approach

For this analysis, I'm combining mapping, statistics, and interactive visualizations to dig into the data. I pull information from two main sources: the NYC 311 API for all the complaint and response data, and the Census Bureau's American Community Survey for neighborhood income levels. After matching up complaints with census tracts, I calculate how long it takes to resolve issues in different areas. Then I use bootstrap statistical methods to test whether the differences I see are real patterns or just random noise. At the end, I built an interactive dashboard so you can explore the patterns yourself across different complaint types and time periods.

## Data Acquisition and Processing

### Data Sources and Acquisition Strategy

I'm pulling data from two sources, both through APIs so anyone can reproduce this analysis later:
...
The NYC 311 API lets you access millions of service requests directly instead of downloading a big static file. I'm querying it using the Socrata format to get exactly what I need:
...
To figure out how wealthy different neighborhoods are, I use the American Community Survey data from the Census Bureau. Specifically, I'm looking at median household income by census tract:
...
For people running this without a Census API key, I added a fallback option using simulated data:
...
Now that I have the raw data, time to clean it up and get it ready for analysis:
...
#### Spatial Join to Link Complaints with Income Data

Here I'm matching each 311 complaint with the closest census tract so I can connect service requests to neighborhood income levels:
...
Before jumping into formal statistics, I want to look around the data to see what patterns emerge and check for any weird outliers:
...
The distribution is heavily skewed to the right. Most requests get closed pretty quickly, but there's a long tail of cases that take weeks or even months. This is why I use the median instead of the mean for my main analysis since medians aren't as affected by extreme values.
...
What I'm seeing here is pretty clear: there's a **negative relationship between income and response time**. Wealthier areas get faster service. But there's also a lot of variation within income levels, which tells me other things matter too like what type of complaint it is or which agency handles it.
...
To really test if these differences are real or just random variation, I'm using **bootstrap resampling** to build confidence intervals for the gap in median response times between rich and poor neighborhoods. Bootstrap is great for this because:

1. **Works with any distribution**: Unlike t tests that assume your data is normally distributed
2. **Perfect for medians**: It directly estimates uncertainty in the statistic I care about
3. **Easy to understand**: The results are in days, not some abstract statistical unit
...
The bootstrap results give us **strong evidence that the disparity is real**. The confidence interval doesn't include zero, so we can rule out the possibility that this is just sampling noise. Poor neighborhoods really do wait longer.

### Looking at Different Types of Complaints

Not all 311 complaints might show the same patterns. Maybe noise complaints get handled pretty equally across rich and poor areas, but housing issues show bigger gaps. Let's break it down:
...
The citywide numbers are useful, but I want to know which specific boroughs have the worst disparities:
...
I built an interactive dashboard so you can dig into the data yourself and look at different neighborhoods, complaint types, or time periods:
...
Here's what the analysis shows. NYC has **real and significant disparities** in how quickly 311 responds based on how much money a neighborhood has:

1. **The Overall Gap**: People in the poorest neighborhoods wait about `r round(observed_diff, 1)` days longer than people in the richest neighborhoods (95% CI: [`r round(ci_lower, 1)`, `r round(ci_upper, 1)`] days). That's roughly a `r round(100 * observed_diff / median(complaints_with_income$response_days[complaints_with_income$income_quartile == "Highest Income"], na.rm = TRUE), 0)`% longer wait time.

2. **It Depends on the Service**: Not all complaint types show the same pattern. Emergency repairs seem to get handled more equally, but quality of life issues show bigger income based gaps.

3. **Different Stories in Different Boroughs**: All five boroughs have these disparities, but they're not all equally bad. The Bronx, which is already the poorest borough, shows the worst gaps. Low income neighborhoods there wait way longer across almost every type of service.

4. **The Extremes Matter Most**: The relationship between income and wait time is strongest when you compare the richest and poorest areas. Middle income neighborhoods fall somewhere in between, but the steepest drop off happens at the top and bottom of the income scale.

### How This Connects to Our Team's Research

My analysis answers a piece of our team's bigger question about fairness in NYC 311 service. What I found is pretty clear: **money buys you faster service**. Even though 311 is supposed to be this democratic system where everyone has equal access, rich and poor New Yorkers don't get the same treatment.

Why does this matter?

**Where to Send Resources**: If poor neighborhoods are waiting longer, maybe they don't have enough city workers assigned there. The city could use this data to send more people to those areas or change how they decide which requests to handle first.

**Things Add Up**: When you're already dealing with inequality, service delays make it worse. If you're wealthy and the city won't fix your pothole, you can just hire someone to do it. If you're poor, you have no choice but to wait.

**Trust Breaks Down**: When people see that calling 311 doesn't really help, they stop trusting the government. And in communities that already face a lot of barriers, that lack of trust can spread to other types of civic engagement too.

**Targeted Solutions**: Knowing where the worst gaps are, both by location and by service type, lets the city focus on fixing specific problems rather than trying to overhaul everything at once.

### Limitations and Caveats

I should be honest about what this analysis can and can't tell us:

1. **Correlation vs Causation**: I found a relationship between income and response time, but that doesn't prove income causes the delays. Rich neighborhoods might also have better political connections or more social capital, and those could be the real drivers.

2. **We Don't Know How Hard the Problems Are**: Maybe poor neighborhoods file more complicated requests that genuinely take longer to fix. Without knowing the actual difficulty of each complaint, it's hard to say if the delays are unfair or just reflect reality.

3. **Location Data Isn't Perfect**: The geographic matching I did is approximate. Some complaints might be assigned to the wrong census tract, which adds noise to the analysis.

4. **Just One Year of Data**: I only looked at 2023. Things might have changed, and COVID probably still affects how the city operates, so this year might not be typical.

5. **Other Things Could Explain This**: Staffing levels, how old the infrastructure is, how dense the population is... all of these vary by neighborhood and could confuse the relationship between income and response time.

### Where to Go From Here

This analysis raises some questions that would be interesting to explore:

- **Track Changes Over Time**: Are these gaps getting bigger, staying the same, or actually shrinking?
- **Break It Down by Agency**: Different city agencies handle different types of complaints. Maybe some are fairer than others.
- **Prove Causation**: Use fancier statistical methods to really prove that income causes the delays, not just correlates with them.
- **Talk to People**: Numbers tell part of the story, but interviewing residents in different neighborhoods could add context and detail.
- **Predict Problems**: Build a model that could flag which complaints are at risk of taking forever, so the city could intervene early.

## Conclusion

I set out to answer this question: **Which boroughs or neighborhoods show the biggest disparities in 311 response times when you compare them by wealth?** The answer is straightforward: **poor neighborhoods in all five boroughs, especially the Bronx, wait way longer** for their 311 complaints to get resolved.

Using bootstrap statistics, interactive maps, and breaking things down by complaint type, I showed that these gaps are:

- **Big**: We're talking about differences measured in days, not hours
- **Real**: Way beyond what you'd expect from random chance
- **Everywhere**: They show up across different services and different parts of the city
- **Fixable**: The city could actually do something about this if they wanted to

My work fits into what our team is studying about fairness in NYC services. While my teammates are looking at other angles like geographic access or demographic patterns, I focused on the money question. And here's what I found: **in New York City, where you live and how much money you have doesn't just determine your address. It determines how fast the government helps you when you ask**.

The 311 system was supposed to level the playing field. It was supposed to give everyone the same access to city services no matter how rich or connected they were. My analysis shows we're not there yet. Fixing this would take real work. The city would need to move resources around, change how they prioritize requests, and actually pay attention to equity numbers. But you can't fix a problem until you measure it first. This analysis gives us a clear starting point to track whether things are getting better or worse.

---

## Appendix: Code Reproducibility

All the code here should work if you run it yourself. To reproduce this analysis:

1. Clone the repository with this `.qmd` file
2. Set your Census API key: `census_api_key("YOUR_KEY", install = TRUE)`
3. Render it: `quarto render individual_report.qmd`

Everything pulls from the APIs automatically, so you don't need to download any files manually.
