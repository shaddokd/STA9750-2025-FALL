...
# Build response metrics by ZIP code
response_by_zip <- nyc_311_clean %>%
  filter(!is.na(incident_zip)) %>%
  mutate(
    # Convert response time to hours for "fastest" table
    response_time_hours = response_time_days * 24
  ) %>%
  group_by(incident_zip, borough) %>%
  summarize(
    n_complaints        = n(),
    mean_response_hours = mean(response_time_hours, na.rm = TRUE),
    sd_response_hours   = sd(response_time_hours, na.rm = TRUE),
    p25_hours           = quantile(response_time_hours, 0.25, na.rm = TRUE),
    p75_hours           = quantile(response_time_hours, 0.75, na.rm = TRUE),
    mean_response_days  = mean(response_time_days, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  arrange(desc(mean_response_days))
...
datatable(
  ...
  caption = "10 Fastest ZIP Codes (Mean Response Time in Hours)",
